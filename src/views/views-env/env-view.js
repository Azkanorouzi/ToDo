...
class EnvironmentView extends ContainerTaskView {
  ...
  _generateHTML() {
    ...
    <section
    class="${
      this._assets.star ? '' : `bg-theme-${this._assets.curTheme}-main`
    } flex flex-col gap-6 transition-transform absolute top-0 left-0 right-0 bottom-0 z-10 ${
      this._assets.star ? '' : `lg:bg-theme-${this._assets.curTheme}-second`
    } lg:overflow-scroll"
    data-theme="true"
  ...
  >
    ...
    <nav
      ...
      }-main p-6 flex justify-between fixed top-0 left-0 right-0 bg-opacity-30 backdrop-blur-xl lg:sticky lg:justify-center"
      data-theme="true"
    >
      <!-- back button -->
    ...
    <!-- Plus button -->
    <div
      class="rounded-full w-12 h-12 grid place-content-center bg-theme-${
        this._assets.curTheme
      }-main text-3xl text-theme-${
      this._assets.curTheme
    }-forth fixed bottom-7 left-7 z-30 cursor-pointer hover:rotate-180 transition-transform lg:sticky lg:py-5 display-env-plus-btn"
      data-theme="true"
    >
